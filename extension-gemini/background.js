...
// timer, so we ping one while a slow extraction is in flight.
...
async function getSettings() {
  ...
  return {
    ...
    thinking: geminiProvider.DEFAULT_THINKING,
    ...stored
  };
}
...
async function handleCapture(msg, tab) {
  ...
  let events;
  try {
    ({ events } = await geminiProvider.extractEvents(images, ctx, settings));
  } finally {
